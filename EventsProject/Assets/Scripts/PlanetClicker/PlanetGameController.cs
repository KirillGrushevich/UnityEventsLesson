...
public class PlanetGameController : MonoBehaviour
{
    ...
    private IEnumerator Start()
    {
        ...
        while (true)
        {
            ...
            for (int i = 0; i < planetsCount; i++)
            {
                ...
                {
                    Debug.Log($"Number{habitablePlanetNumber}" + " Habi created");
                    planets[i].Setup(habitablePlanets[Random.Range(0, habitablePlanets.Length)], OnHabitablePlanetClicked);
                }

                else if (Random.Range(0, 3) == 0)
                {
                    planets[i].Setup(uninhabitablePlanets[Random.Range(0, uninhabitablePlanets.Length)], OnUnhabitablePlanetClicked);
                }
                else
                {
                    planets[i].Setup(gasGiants[Random.Range(0, gasGiants.Length)], OnGazgiantPlanetClicked);
                }
    ...
    private void OnUnhabitablePlanetClicked()
    {
        Debug.Log("OnUnhabitablePlanetClicked");
    }
    private void OnGazgiantPlanetClicked()
    {
        Debug.Log("OnGazgiantPlanetClicked");
    }
